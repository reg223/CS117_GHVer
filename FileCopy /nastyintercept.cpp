...
copyFile(string sourceDir, string fileName, string targetDir, int nastiness) {
  ...
  try {
    ...
    char *buffer1 = (char *)malloc(sourceSize/2);
    char *buffer2 = (char *)malloc(sourceSize - sourceSize/2);

    len = inputFile.fread(buffer1, 1, sourceSize/2);
    len += inputFile.fread(buffer2, 1, sourceSize - sourceSize/2);

    for (size_t i = 0; i <sourceSize/2; i++) {
      buffer[i] = buffer1[i];
    }
    for (size_t i = 0; i <sourceSize - sourceSize/2; i++) {
      buffer[i+sourceSize/2] = buffer2[i];
    }
