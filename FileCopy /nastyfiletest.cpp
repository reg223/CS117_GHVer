...
main(int argc, char *argv[]) {
  ...
  while ((sourceFile = readdir(SRC)) != NULL) {
    ...
    // skip subdirectories
    for (int i  = 0; i < 10; i++){
      copyFile(argv[2], sourceFile->d_name,  argv[3], nastiness, i);
    }
...
void
copyFile(string sourceDir, string fileName, string targetDir, int nastiness, int count) {
  ...
  string fn = fileName + to_string(count);
  string targetName = makeFileName(targetDir, fn);
